...
describe('DropdownField JSON Schema Validation (via DropdownField)', () => {
  ...
  describe('type', () => {
    it('Should pass when type is dropdown', () => {
      const field = makeBaseField({ type: 'dropdown' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('value', () => {
    it('Should pass when value is a string', () => {
      const field = makeBaseField({ value: 'option_1' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

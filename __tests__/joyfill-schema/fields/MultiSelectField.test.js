...
describe('MultiSelectField JSON Schema Validation (via MultiSelectField)', () => {

  describe('general', () => {
    it('Should pass with options array and string value.', () => {
      const fields = [{ _id: 'f3', file: 'file_1', type: 'multiSelect', options: [{ _id: 'o1', value: 'A' }], value: ['A'] }];
  ...
  describe('forward compatibility', () => {
    ...
    it('Should pass when option has unknown properties present', () => {
      const field = makeBaseField({
        options: [
          { _id: 'o1', value: 'option_1', foo: 'bar', extra: 123 }
        ]
      });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
  ...
  describe('type', () => {
    it('Should pass when type is multiSelect', () => {
      const field = makeBaseField({ type: 'multiSelect' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('value', () => {
    it('Should pass when value is an array of strings', () => {
      const field = makeBaseField({ value: ['option_1', 'option_2'] });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('options', () => {
    ...
    it('Should pass when options is an array of objects with required properties', () => {
      const field = makeBaseField({ options: [{ _id: 'o1', value: 'option_1' }, { _id: 'o2', value: 'option_2' }] });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should pass when options is an empty array', () => {
      const field = makeBaseField({ options: [] });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    }); 
    ...
    it('Should fail when options is not an array', () => {
      const field = makeBaseField({ options: 'not-an-array' });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });

    it('Should fail when options contains a non-object', () => {
      const field = makeBaseField({ options: [{ _id: 'o1', value: 'option_1', foo: 'bar', extra: 123 }, 'not-an-object'] });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });

    it('Should pass when options contains an object with unknown properties', () => {
      const field = makeBaseField({ options: [{ _id: 'o1', value: 'option_1', foo: 'bar', extra: 123 }] });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
    
    it('Should fail when options are missing required properties', () => {
      // Missing _id
      let field = makeBaseField({ options: [{ value: 'option_1' }] });
      let { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);

      // Missing value
      field = makeBaseField({ options: [{ _id: 'o1' }] });
      ({ isValid } = runValidation(baseDoc([field])));
      expect(isValid).toBe(false);

      // Missing both _id and value
      field = makeBaseField({ options: [{}] });
      ({ isValid } = runValidation(baseDoc([field])));
      expect(isValid).toBe(false);
    });
  });

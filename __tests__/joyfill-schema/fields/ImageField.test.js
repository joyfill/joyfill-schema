...
function makeBaseField(overrides = {}) {
  return {
    ...
    file: 'file_1',
    type: 'image',
    value: [{ _id: 'i1', url: 'http://example.com/image.png' }],
    ...overrides
  };
}
...
describe('ImageField JSON Schema Validation (via ImageField)', () => {

  describe('general', () => {
    it('Should pass with options array and string value.', () => {
      const fields = [{ _id: 'f3', file: 'file_1', type: 'image', value: [{ _id: 'i1', url: 'http://example.com/image.png' }] }];
  ...
  describe('forward compatibility', () => {
    ...
    it('Should pass when value has unknown properties present', () => {
      const field = makeBaseField({
        value: [
          { _id: 'i1', url: 'http://example.com/image.png', foo: 'bar', extra: 123 }
        ]
      });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
  ...
  describe('type', () => {
    it('Should pass when type is image', () => {
      const field = makeBaseField({ type: 'image' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('value', () => {
    it('Should pass when value is an array of objects with required properties', () => {
      const field = makeBaseField({ value: [{ _id: 'i1', url: 'http://example.com/image.png' }, { _id: 'i2', url: 'http://example.com/image2.png' }] });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
    ...
    it('Should fail when value is not an array of objects with required properties', () => {
      const field1 = makeBaseField({ value: [{ url: 'http://example.com/image.png' }] });

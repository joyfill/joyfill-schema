...
function makeBaseField(overrides = {}) {
  return {
    ...
    type: 'chart',
    value: [
      { _id: 's1', points: [{ _id: 'p1', x: 0, y: 0 }, { _id: 'p2', x: 1, y: 1 }] },
      { _id: 's2', points: [{ _id: 'p3', x: 2, y: 2 }, { _id: 'p4', x: 3, y: 3 }] }
    ],
    yTitle: 'Y', 
    yMax: 10, 
    yMin: 0,
...
describe('FileField JSON Schema Validation (via FileField)', () => {
  ...
  describe('forward compatibility', () => {
    ...
    it('Should pass when ChartSeries has unknown properties', () => {
    ...
    it('Should pass when ChartPoint has unknown properties', () => {
      const field = makeBaseField({
        value: [
          {
            _id: 's1',
            points: [
              { _id: 'p1', x: 1, y: 2, foo: 'bar', extra: 123 },
              { _id: 'p2', x: 2, y: 3 }
            ]
          }
        ]
  ...
  describe('type', () => {
  it('Should pass when type is chart', () => {
      const field = makeBaseField({ type: 'chart' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('value', () => {
    ...
    it('Should fail when value is not an array of objects with required properties', () => {
      const field1 = makeBaseField({ value: [{ title: 'Series 1' }] });
  ...
  describe('yTitle', () => {
    it('Should pass when yTitle is a string', () => {
      const field = makeBaseField({ yTitle: 'Y' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when yTitle is not a string', () => {
      const field = makeBaseField({ yTitle: 123 });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('yMax', () => {
    it('Should pass when yMax is a number', () => {
      const field = makeBaseField({ yMax: 10 });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when yMax is not a number', () => {
      const field = makeBaseField({ yMax: 'not-a-number' });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('yMin', () => {
    it('Should pass when yMin is a number', () => {
      const field = makeBaseField({ yMin: 0 });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when yMin is not a number', () => {
      const field = makeBaseField({ yMin: 'not-a-number' });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('xTitle', () => {
    it('Should pass when xTitle is a string', () => {
      const field = makeBaseField({ xTitle: 'X' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when xTitle is not a string', () => {
      const field = makeBaseField({ xTitle: 123 });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('xMax', () => {
    it('Should pass when xMax is a number', () => {
      const field = makeBaseField({ xMax: 10 });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when xMax is not a number', () => {
      const field = makeBaseField({ xMax: 'not-a-number' });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });
  ...
  describe('xMin', () => {
    it('Should pass when xMin is a number', () => {
      const field = makeBaseField({ xMin: 0 });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

    it('Should fail when xMin is not a number', () => {
      const field = makeBaseField({ xMin: 'not-a-number' });
      const { isValid } = runValidation(baseDoc([field]));
      expect(isValid).toBe(false);
    });
  });

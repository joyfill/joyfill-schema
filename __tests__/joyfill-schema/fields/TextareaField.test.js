...
describe('TextareaField JSON Schema Validation (via TextareaField)', () => {

  describe('general', () => {
    it('Should pass with minimal valid field (required base props only).', () => {
      const fields = [{ _id: 'bf1', file: 'file_1', type: 'textarea' }];
  ...
  describe('type', () => {
    it('Should pass when type is textarea', () => {
      const field = makeBaseField({ type: 'textarea' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
  ...
  describe('value', () => {
    it('Should pass when value is a string', () => {
      const field = makeBaseField({ value: 'Hello, world!' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

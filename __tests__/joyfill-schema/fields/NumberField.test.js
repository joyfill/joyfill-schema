...
describe('NumberField JSON Schema Validation (via NumberField)', () => {

  describe('general', () => {
    it('Should pass with minimal valid field (required base props only).', () => {
      const fields = [{ _id: 'bf1', file: 'file_1', type: 'number' }];
  ...
  describe('type', () => {
    it('Should pass when type is number', () => {
      const field = makeBaseField({ type: 'number' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });

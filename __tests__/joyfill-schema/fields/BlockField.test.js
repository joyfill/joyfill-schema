...
describe('BlockField JSON Schema Validation (via BlockField)', () => {

  describe('general', () => {
    it('Should pass with minimal valid field (required base props only).', () => {
      const fields = [{ _id: 'bf1', file: 'file_1', type: 'block' }];
  ...
  describe('type', () => {
    it('Should pass when type is block', () => {
      const field = makeBaseField({ type: 'block' });
      const { isValid, errors } = runValidation(baseDoc([field]));
      if (!isValid) console.error(errors);
      expect(isValid).toBe(true);
    });
